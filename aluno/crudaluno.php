...
if (isset($_GET['excluir'])) {
    ...
    $sql_nome = "SELECT nomealuno FROM Aluno WHERE idaluno = :idaluno";
    $stmt_nome = $conexao->prepare($sql_nome);
    $stmt_nome->bindParam(':idaluno', $idaluno, PDO::PARAM_INT);
    $stmt_nome->execute();
    $resultado_nome = $stmt_nome->fetch(PDO::FETCH_ASSOC);
    $nomealuno = $resultado_nome['nomealuno'];

    // Mostra um script de confirmação em JavaScript antes de executar a exclusão
    echo "<script>
        var confirmar = confirm('Tem certeza que deseja excluir o aluno $nomealuno?');
        if (confirmar) {
            window.location.href = 'excluiraluno.php?idaluno=$idaluno'; 
        } else {
            window.location.href = 'listaaluno.php';
        }
    </script>";
